...
// ── SYSTEM PAUSE STATE ────────────────────────────────────────────────────────
// In-memory flag — persists as long as dashboard is running
// Agents poll /api/pause-status before every send
let SYSTEM_PAUSED = false;

app.get('/api/pause-status', (req, res) => {
  res.json({ paused: SYSTEM_PAUSED, ts: new Date().toISOString() });
});

app.post('/api/pause', (req, res) => {
  SYSTEM_PAUSED = true;
  console.log('[System] ⏸️  PAUSED by dashboard');
  pushEvent('agent', 'system_paused', { contact: 'System paused via dashboard' });
  res.json({ ok: true, paused: true });
});

app.post('/api/resume', (req, res) => {
  SYSTEM_PAUSED = false;
  console.log('[System] ▶️  RESUMED by dashboard');
  pushEvent('agent', 'system_resumed', { contact: 'System resumed via dashboard' });
